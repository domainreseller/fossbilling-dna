...
⬇️ **احصل على أحدث نسخة مُختبَرة من هنا: https://github.com/domainreseller/fossbilling-dna/releases/latest**

> ⚠️ **لا** تستخدم الزر الأخضر **Code → Download ZIP** — فهو يقوم بتنزيل فرع التطوير الخام. حزم الإصدارات مُرقّمة ومُختبَرة وجاهزة للاستخدام الفعلي.
...
كلاهما مدعوم — أدخلهما في نفس حقلي الوحدة؛ وتكتشف الوحدة تلقائيًا أي واجهة API يجب استخدامها:

| ما لديك | حقل "اسم المستخدم" | حقل "كلمة المرور" | واجهة API المستخدمة |
|---|---|---|---|
| **بيانات اللوحة الجديدة** (موصى به) | Reseller ID — معرف UUID مثل `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx` | API Key | REST |
| **البيانات القديمة (Legacy)** | اسم مستخدم API | كلمة مرور API | SOAP |

> 💡 ستجد **Reseller ID** و **API Key** في لوحة تحكم DomainNameAPI ضمن قسم **إعدادات API**.
> ⚠️ هذه **بيانات اعتماد API** — البريد الإلكتروني وكلمة المرور الخاصان بتسجيل الدخول إلى اللوحة **لن يعملا** هنا.

لا حاجة إلى أي إعداد إضافي — إذا كان حقل اسم المستخدم يحتوي على UUID فستستخدم الوحدة واجهة REST الحديثة، وإلا فستستخدم SOAP الكلاسيكية.
